...
def preprocess_physionet():
    ...
    # read label
    label_df = pd.read_csv(r'C:\Users\Kimia\Downloads\GIthub exercise\resnet1d\dataset\REFERENCE-v3.csv', header=None)
    label = label_df.iloc[:,1].values
    print(Counter(label))

    # read data
    all_data = []
    filenames = pd.read_csv(r"C:\Users\Kimia\Downloads\GIthub exercise\resnet1d\dataset\training2017\RECORDS", header=None)
    filenames = filenames.iloc[:,0].values
    print(filenames)
    for filename in tqdm(filenames):
        mat = scipy.io.loadmat(r'C:\Users\Kimia\Downloads\GIthub exercise\resnet1d\dataset\training2017\{0}.mat'.format(filename))
        mat = np.array(mat['val'])[0]
        all_data.append(mat)
    all_data = np.array(all_data)

    res = {'data':all_data, 'label':label}
    with open(r"C:\Users\Kimia\Downloads\GIthub exercise\resnet1d\dataset\challenge2017.pkl", 'wb') as fout:
...
def read_data_physionet_2_clean_federated(m_clients, test_ratio=0.2, window_size=3000, stride=500):
    ...
    # read pkl
    with open(r"C:\Users\Kimia\Downloads\GIthub exercise\resnet1d\dataset\challenge2017.pkl", 'rb') as fin:
        res = pickle.load(fin)
    ## scale data
    all_data = res['data']
